...
int   nfil = 1;      /* the layer number where FD ends: Ling 01/30/04 */
int   inlay = 1;     /* downward or upward in FD calculation: Ling 01/31/04 */
...
/* for check only */
   ...
   int iz = 20, izz0 = 20;  /*output point of kirchhoff */
   float zcore;  /* the distance of the kiroutput from the GRT-FD interface */
   int rcore = 1;
   float ztop = 4;
...
/* Ling: 03/01/2004 */
   float zbeg, xbeg;
   float *outplot;
   int izbeg, ixbeg, idzplot, idxplot;
   

   int kirrecord_n(struct vel_stress *p, int *ptable, float *na, int izz0, int iz, 
	  float h, float dt, int nx, int nl, int nl_skip, int nh, int kdx, 
	  FILE *kirout, int source);

   int kirrecord_1(struct vel_stress *p, int *ptable, float *na, int izz0, int iz, 
	  float h, float dt, int nx, int nl, int nl_skip, int nh, int kdx, 
	  FILE *kirout, int source);
   
   int kirrecord(struct vel_stress *p, int *ptable, float *na, int iz, 
	  float h, float dt, int nx, int nl, int nl_skip, int nh, int kdx, 
	  FILE *kirout, int source, int *outind);
...
/* nfil is the layer number where FD ends: Ling 01/30/04 */
    getpar("nfil",    "d", &nfil);
    getpar("ntt",     "d", &ntt);    /* Ling 02/10/04 */
   ...
   if(xtrace > 0 && ztrace > 0 && plot_trace) {
     mstpar("zbeg",        "f", &zbeg);
...
//      fprintf(stderr,"cannot allocate p1 memory\n"); 
      printf("cannot allocate p1 memory\n"); 
      exit(-1); 
   }
...
/* following for the interpolation */
   ...
   printf("nx = %d, nz = %d, inlay = %d\n",nx,nz,inlay);   /* Ling: 01/31/04 */
   printf("mtotal = %d\n",mtotal);   /* Ling: 01/31/04 */
   ...
   ntt = (int) (((float)((ntt-3)*dp))/dt);    /* Ling: 02/10/04 */
   printf("nt = %d   ntt = %d\n",nt,ntt);
...
*/
   ...
   izz0 = iz;
   iz = (iz>nz-2) ? nz-2: iz;
   izz0 = (izz0>nz) ? nz: izz0;
   izt = -(int)(ztop*6371.0/3480.0/h);    /* ztop km above the interface */
   izt = -(int)(ztop/h);    /* ztop km above the interface **/

/* Ling: 01/31/04 */
//   iz  = (int) (zcore/h);     /* zcore km away from the GRT-FD interface */
//   iz  = ((iz>nz2/2) ? nz2/2: iz);
//   izt = -(int)(ztop/h);    /* ztop km below the interface */
//   fprintf(stderr,"iz = %d,  izt = %d\n",iz,izt);

   nh1 = nh;
   if(nh > -nd - 7) nh1 = -nd - 7;
   printf("nd = %d  iz = %d  izz0 = %d  izt = %d\n",nd,iz,izz0,izt);
...
//   for(it=0; it< nt; it++) {
   for(it=0; it< ntt; it++) {
	 ...
	 if(!(it % 100)) printf("it = %d,  nt = %d\n",it,ntt-1); 
         if(it<nt)interpl(infile,rec_len,it,rdt,mtotal,ns,sy,uwt34,nx-nl,nz+1-nd);
         tstep(pp1,ptable,ntable,it);

         if(!(it % kdt)){
             if(rcore != 0) {
                 j += 1;
	         if(!(j % 50)) printf("it = %d,  j = %d\n",it,j); 
//                 kirrecord(pp1,ptable,ntable,iz,h,dt,nx,nl,nl_skip,nh,kdx,fop_kir,source,&outind);
//                 if(outind<knx-1)break;
	       if(source>=0) {
                 kirrecord_n(pp1,ptable,ntable,izz0,iz,h,dt,nx,nl,nl_skip,nh,kdx,fop_kir,source);
	       } else {
	         kirrecord_1(pp1,ptable,ntable,izz0,iz,h,dt,nx,nl,nl_skip,nh,kdx,fop_kir,source);
	       }
             }
	     if(topp != 0)
                 kirrecord(pp1,ptable,ntable,izt,h,dt,nx,nl,nl_skip,nh,kdx,fop_topp,0,&outind);
	     if(tops != 0)
                 kirrecord(pp1,ptable,ntable,izt,h,dt,nx,nl,nl_skip,nh,kdx,fop_tops,1,&outind);
...
/* my              
	      for(itrace = 0; itrace < xtrace*ztrace; itrace++) {
//		printf("%d  %d  %d\n",itrace,iz1[itrace],ix1[itrace]);
	        outplot[itrace] = pp1[iz1[itrace]*nx+ix1[itrace]].u;
//		printf("%d  %d  %d  outu = %f\n",itrace,iz1[itrace],ix1[itrace],pp1[iz1[itrace]*nx+ix1[itrace]].u);
	        fwrite(outplot,sizeof(float),xtrace*ztrace,fopu1);
	      }
              for(itrace = 0; itrace < xtrace*ztrace; itrace++) {
	        outplot[itrace] = pp1[iz1[itrace]*nx+ix1[itrace]].w;
	        fwrite(outplot,sizeof(float),xtrace*ztrace,fopw1);
	      }
my */	    
	        kirrecord_my(pp1,nx,iz1,ix1,xtrace,ztrace,fopu1,fopw1,0);
	    }    
...
void tstep(p1,ptable,ntable,it)
int *ptable, it;
float *ntable;
struct vel_stress *p1;
...
//	if(stress==0 && !(it % 100))printf("it = %d\n",it);
      }
...
//my          t12zz = (p1[  0].t12+t124[0]) - p1[-nx].t12;
          t12xx = p1[  1].t12 - p1[  0].t12;
...
//my              wxx   = (p1[ 0].w+w0[0])-p1[ -1].w;
/* Ling: 01/31/2004  */
      uzz *= (float) inlay;

              if(afilter>0 && iz>=nz-nh+1)
                  wxx *= pfilter[(iz-(nz-nh+1))*2 + 1];
              if(afilter >2 && iz<nd+nh1)
...
//my              t12xx = p1[  1].t12 - (p1[  0].t12-t120[0]);
              t22zz = p1[ nx].t22 - p1[  0].t22;
	      if(iz == 0) t22zz -= t22nl;
/* Ling: 01/31/2004  */
       t12zz *= (float) inlay;
...

{
   ...
   FILE *ftmp;
   int x0, x1, z0, z1;
   int nout,nlay1,nlay2,nlaynum,nab,nae;       /* Ling: 01/31/04 */
...
/**   nfil = nfinal(jo,ss,nrec);  **/
   ...
   if(nd > nz2){
       printf("nd change from %d to %d\n",nd,nz2);
       nd = nz2;
   }

   nz2 = (int) ((thickness+1.e-3)/(0.5*h));
   printf("thick = %f,  nz2 = %d\n",thickness,nz2);
   
   ivp = cc[nrec-1]; ivs = ss[nrec-1]; iden= dd[nrec-1];
/**   fvp = cc[nfil-1];                   fden= dd[nfil-1];    **/
   nout = ((nfil>nrec) ? nfil : nfil-2);
   nout = ((nout>=0) ? nout: 0);
   fvp = cc[nout]; fvs = ss[nout]; fden= dd[nout];
   printf("ivp = %f, fvp = %f\n",ivp,fvp);
...
/* Ling: 02/11/04 */
   ...
   *pptable= (int   *) malloc(nx2*(nz2+nf2+1+nd2+4)/4*sizeof(int));
   ...
   { 
      printf("cannot allocate a, b, c memory\n"); 
      exit(-1); 
   }
   nd2 *= (-1);
   nd  *= (-1);
   ...
   cnt=-nd2*nx2+2*nx2;
   ivp *= ivp; ivs *= ivs;
/**   ivp = ((nfil>nrec) ? ivp*ivp: fvp*fvp);
   ivs = ((nfil>nrec) ? ivs*ivs: fvs*fvs);
   iden = ((nfil>nrec) ? iden: fden);   **/
...
/**        for(k=0; k<nll; k++){ 
        ...
        for(i=2*nl; i<nx2; i++){
	        ...
	        if(iz2 > nz2 + nf2) iz2=nz2+nf2;
		if(!(i % 100))printf("j = %d, iz1 = %d,  iz2 = %d\n",j,iz1,iz2);
...
/*       for(i=nlay2; i<nlay2+400; i++){    */
           ...
           iz2 = (int) ((thickness+1.e-3)/(0.5*h)) + 2;
/** why -5 ?           if(iz2 > nz2 + nf2 -5) iz2=nz2+nf2-5;    **/
           if(iz2 > nz2 + nf2) iz2=nz2+nf2;
   ...
   if(fac>=1.0){
      printf("fac= %4.2f< instability !\n",fac);
      printf("vpmax= %4.2f at iz=%d ix=%d\n",vpmax, j/nx2/2, j-2*nx2*(j/nx2/2));
      exit(-1);
   }
   else{
      printf("fac= %4.2f, stable\n",fac);
   }
...
/* Encode media parameters in table */
   printf("encode media parameters \n");
 ...
 /* skip this if inputing random model */
	   ...
	   for(j2=0; j2<4; j2++){
		...
		if(len>=5*NTSIZE){
		    ...
		    *ntable = (float *) realloc((char *)na,syze);
		    if(*ntable==NULL){
			printf("redimensioning ntable fails (try again)\n");
		        NTSIZE *= 0.55;
		        syze = NTSIZE * 5 * sizeof(float);
		        printf( 
		        "redimensioning ntable to %d entries. (%d bytes, %.2f Mbytes)\n" 
		        ...
		        *ntable = (float *) realloc((char *)na,syze);
		        if(*ntable==NULL){
			    printf("Memory re-allocation for ntable fails (quit)\n");
			    exit (-3);
	                }
	            }
   ...
   printf("finish the forth order portion, now start edges\n");
   /* tabulate edges and                                      */
...
*/
	 ...
	 ta(0)=mu;
	    ...
	    NTSIZE += nx2+2*(nz-nd);
	    syze = NTSIZE * 5 * sizeof(float);
	    printf("redimensioning ntable to %d entries. (%d bytes, %.2f Mbytes)\n"
	    ,NTSIZE,syze,syze/1000000.);
	    *ntable = (float *) realloc((char *)na,syze);
	    if (*ntable==NULL) {
	       printf("Memory re-allocation for ntable fails!\n");
	       exit(-3);
	    }
	    if (*ntable != na) copy(na,*ntable,len);
	    na = *ntable;
   ...
   printf("done with table formation, %d types of media! \n",len/5);
   NTSIZE = len/5 + 1;
   syze = NTSIZE * 5 * sizeof(float);
   printf("final ntable size: %d entries. (%d bytes, %.2f Mbytes)\n"
   ,NTSIZE,syze,syze/1000000.);
...
kirrecord_n(struct vel_stress *p, int *ptable, float *na, int izz0, int iz, 
	  float h, float dt, int nx, int nl, int nl_skip, int nh, int kdx, 
	  FILE *kirout, int source)

/* source = 0   -> velocity Vx and Vz are recorded */ 
...

{
	...
	for(i=0; i< knx; i++)		/* u velocity */
	{
	    offset   = nl+1 + (i+1+nl_skip)*kdx +izz0*nx;
	    pt       = ptable + offset;
	    pp       = p      + offset;
	    
            hold[i]  =  pp[0].u;
//	    if(!(hold[i]==0.f))printf("%d   u = %f\n",i,hold[i]);
	}
...
//	    if(!(hold[i]==0.f))printf("%d   t22 = %f\n",i,hold[i]);
	...
	for(i=0; i< knx; i++)		/* t12 */
	{
	    ...
	    pp       = p      + offset;
            ...
            hold[i]  =  pp[0].t12;
//	    if(!(hold[i]==0.f))printf("%d   t12 = %f\n",i,hold[i]);
	}
	fwrite(hold,sizeof(float),knx,kirout);
	for(i=0; i< knx; i++)		/* w velocity */
	{
	    offset   = nl+1 + (i+1+nl_skip)*kdx +izz0*nx;
	    pt       = ptable + offset;
	    pp       = p      + offset;
...
*/	
	for(i=0; i< knx; i++)		/* (gra div u)z */
	{
	    ...
	    hold[i] *=  9.0/8.0*0.5/(ta(0)+tb(0))*dt/h/h;          
//	    hold[i] *= (float)inlay;      /* Ling: 01/31/04 */         
	}
	fwrite(hold,sizeof(float),knx,kirout);
/*	
//	for(i=0; i< knx; i++)
...
//                          -pp[1-nx].w+pp[-1-nx].w)*(float)inlay/4.0
...
//	for(i=0; i< knx; i++)
	{
	    ...
	    hold[i] =  ( (  pp[1+nx].w-2.*pp[1].w+pp[1-nx].w 
	                ...
	                -(  pp[2*nx].u-2.*pp[nx].u+pp[0].u + 
	                  ...
	                  -(pp[nx].u-2.*pp[0].u+pp[-nx].u +
	                    pp[nx+1].u-2.*pp[1].u+pp[-nx+1].u) )*(float)inlay)/2./h/h/h;
	}
	fwrite(hold,sizeof(float),knx,kirout);
*/	
...
//	    hold[i] = ( (pp[nx+1].u-pp[nx].u-pp[1].u+pp[0].u)*(float)inlay/2.
	    ...
	    pp       = p      + offset;
            ...
            hold[i] = ( ((pp[2*nx+1].u-pp[2*nx].u-pp[nx+1].u+pp[nx].u)
                       ...
                       +((pp[1+nx].w-2.*pp[nx].w+pp[nx-1].w)
                        -(pp[1-nx].w-2.*pp[-nx].w+pp[-nx-1].w))*(float)inlay/2.)/h/h/h;
...
kirrecord_1(struct vel_stress *p, int *ptable, float *na, int izz0, int iz, 
	  float h, float dt, int nx, int nl, int nl_skip, int nh, int kdx, 
	  FILE *kirout, int source)
...
*/
	...
	for(i=0; i< knx; i++)		/* P1 */
	{
	    ...
	    hold[i]  = (pp[  +1].u - pp[  0].u)* (9.0/8.0)
	              -(pp[  +2].u - pp[ -1].u)* (1.0/24.0); 
	    hold[i] /= h; 
	...
	for(i=0; i< knx; i++)		/* P2 */
	{
	    ...
	    hold[i]  = (pp[  nx].w - pp[  0].w)* (9.0/8.0)
	              -(pp[2*nx].w - pp[-nx].w)* (1.0/24.0); 
	    hold[i] /= h; 
	...
	for(i=0; i< knx; i++)		/* S1 */
	{
	    ...
	    hold[i]  = (pp[  nx].u - pp[  0].u)* (9.0/8.0)
	              -(pp[2*nx].u - pp[-nx].u)* (1.0/24.0); 
	    hold[i] /= h;          
//	    hold[i] *= (float)inlay;      /* Ling: 01/31/04 */         
	...
	for(i=0; i< knx; i++)		/* S2 */
	{
	    ...
	    hold[i]  = (pp[  +1].w - pp[  0].w)* (9.0/8.0)
	              -(pp[  +2].w - pp[ -1].w)* (1.0/24.0); 
	    hold[i] /= h;          
//	    hold[i] *= (float)inlay;      /* Ling: 01/31/04 */         
	...
	fwrite(hold,sizeof(float),knx,kirout);
/*	
	for(i=0; i< knx; i++)		// u velocity (+2*nx)
	{
	    offset   = nl+1 + (i+1+nl_skip)*kdx +iz*nx;
	    pt       = ptable + offset;
	    pp       = p      + offset;
	...
	fwrite(hold,sizeof(float),knx,kirout);
	for(i=0; i< knx; i++)		// w velocity (+2*nx)
	{
	    offset   = nl+1 + (i+1+nl_skip)*kdx +iz*nx;
	    pt       = ptable + offset;
	    pp       = p      + offset;
...
kirrecord_bak(struct vel_stress *p, int *ptable, float *na, int iz, 
	  float h, float dt, int nx, int nl, int nl_skip, int nh, int kdx, 
	  FILE *kirout, int source)
...
kirrecord_my(struct vel_stress *p, int nx, int *iz1, int *ix1, int xtrace,
	  int ztrace, FILE *fu, FILE *fw, int source)
