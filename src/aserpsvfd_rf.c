...
/* Last modified: 06/2004 by Ling Chen                            */
/* considering multiples with converted phases at discontinuities */
/* within bouncing points: add Ps in direct rays                  */
/* * have not tested        06/12/04                              */
/*                                                                */
/******************************************************************/
...
int     layincrust = 2;  /* the layer number for the middle crust (PREM)   Ling: 06/10/04 */
int     laycrust = 3;    /* the layer number for the moho (PREM)   Ling: 06/10/04 */
int     lay210 = 12;     /* the layer number for the 210  (PREM)   Ling: 06/10/04 */
int     lay390 = 21;     /* the layer number for the 390  (PREM)   Ling: 06/12/04 */
int     lay670 = 37;     /* the layer number for the 370  (PREM)   Ling: 06/12/04 */
...
/* following for the ray descriptions */
    ...
    int     idirect = 0;     
            /* index for direct rays (if >0, need to consider converted waves 06/12/04 */
...
/* FFT the Gauss source function */
    ...
    free(source);
//    printf("l2n=%d, nx1=%d, ncent=%d, fscl=%f, dp=%f, nconv=%d\n",l2n,nx1,ncent,fscl,dp,nconv);
...
*/
    ...
    thickness=0.0;
    if(nfil>nrec){
        for(n=nrec; n<nfil; n++)       /* Ling: 01/31/04 */
	    thickness += tth[n];
    } else {	    
        for(n=nfil-1; n<nrec-1; n++)     /* Ling: 01/31/04 */
	    thickness += tth[n];
    }	    
...
/**    n =  ((nfil>nrec) ? nfil-2 : nfil);  **/
    ...
    if( nd > n )
	nd = n;
//    fprintf(stderr,"thh=%f, nz=%d nfil=%d nrec=%d\n",thickness,nz,nfil,nrec);
    printf("thh=%f, nz=%d nfil=%d nrec=%d\n",thickness,nz,nfil,nrec);
...
//    fprintf(stderr,"nz=%d nfil=%d nrec=%d\n",nz,nfil,nrec);
    ...
    zlaymax -= 1;
    if(zlaymax < 1) zlaymax = 1;             /* Ling: 02/16/04 */
    printf("zlaymax = %d\n",zlaymax);
...
//	if(nfil<nrec) nfil0 = nfil;          /* Ling: 02/16/04 */
	if(nfil<nrec) nfil0 = 1;          /* Ling: 02/26/04 */
